...
alter table CheckAccount 
   drop foreign key FK_CHECKACC_ACCOUNT_C_ACCOUNT;

alter table Customer_CheckAccount 
   drop foreign key FK_CUSTOMER_CHECKACCO_CHECKACC;

alter table Customer_CheckAccount 
   drop foreign key FK_CUSTOMER_CUSTOMER__CUSTOMER;

alter table Customer_CheckAccount 
   drop foreign key FK_CUSTOMER_SUBBANK_C_SUBBANK;

alter table Customer_DepositAccount 
   drop foreign key FK_CUSTOMER_CUSTOMER__CUSTOMER;

alter table Customer_DepositAccount 
   drop foreign key FK_CUSTOMER_DEPOSITAC_DEPOSITA;

alter table Customer_DepositAccount 
   drop foreign key FK_CUSTOMER_SUBBANK_D_SUBBANK;

alter table Customer_Loan 
   drop foreign key FK_CUSTOMER_CUSTOMER__LOAN;

alter table Customer_Loan 
   drop foreign key FK_CUSTOMER_CUSTOMER__CUSTOMER;

alter table Department 
   drop foreign key FK_DEPARTME_SUBBANK_D_SUBBANK;

alter table DepositAccount 
   drop foreign key FK_DEPOSITA_ACCOUNT_D_ACCOUNT;
...
alter table Employee_Customer 
   drop foreign key FK_EMPLOYEE_EMPLOYEE__CUSTOMER;

alter table Loan 
   drop foreign key FK_LOAN_LOAN_SUBB_SUBBANK;

alter table Payment 
   drop foreign key FK_PAYMENT_PAY_LOAN_LOAN;
...
(
   ...
   Loan_Status			int(1)	comment '发放状态',
   primary key (Loan_ID)
);
...
alter table Customer_DepositAccount add constraint FK_CUSTOMER_CUSTOMER__CUSTOMER foreign key (User_ID)
      references Customer (User_ID) on delete restrict on update restrict;
...
alter table Customer_Loan add constraint FK_CUSTOMER_CUSTOMER__CUSTOMER foreign key (User_ID)
      references Customer (User_ID) on delete restrict on update restrict;
