...
* 技术栈：`MySQL`+`Flask`+`Jinja2`+`html`+`css`等，Pycharm相关设置如下：
...
* 主页为登录/注册页面，可以注册新**客户**用户（**不含员工**用户，员工用户信息在初始化sql文件中插入数据库，且只能修改密码，不可修改其他属性），登录用户分为两类：客户和员工，他们的权限区分如下：
  * 客户用户：
    ...
    * 贷款管理：贷款信息的增、删、查功能
      * “增”指的是申请新的贷款，新增贷款时需要从对应支行的对应部门选择一个员工做自己的账户负责人
